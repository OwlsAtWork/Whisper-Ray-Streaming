...
class SilenceAtEndOfChunk(BufferingStrategyInterface):
    ...
    def __init__(self, client, **kwargs):
        ...
        logger.info(f"Silence threshold: {self.silence_threshold_seconds}s, Min speech: {self.min_speech_seconds}s, Max buffer: {self.max_buffer_seconds}s")
    ...
    def process_audio(self, websocket: WebSocket, vad_handle, asr_handle, debug_output):
        ...
        if self.processing_flag:
            logger.info("Skipping processing: already in progress")
            return
        ...
        if len(self.client.buffer) < min_buffer_size:
            logger.info("Insufficient buffer size, waiting for more data")
            return
    ...
    async def check_silence_and_process(self, websocket: WebSocket, vad_handle, asr_handle, debug_output):
        ...
        if self.processing_flag:
            logger.info("Skipping silence check: already processing")
            return
